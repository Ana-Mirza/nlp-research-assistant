...
% ============================================================================
\begin{abstract}
We present an academic research assistance system that helps researchers explore how their work relates to existing literature. The system employs a hybrid Retrieval-Augmented Generation (RAG) pipeline combining dense retrieval (ChromaDB with sentence-transformers), sparse retrieval (BM25), Reciprocal Rank Fusion, and cross-encoder re-ranking over a corpus of 388K arXiv papers. An open-source LLM generates grounded analyses, structured paper comparisons, state-of-the-art reviews, and research gap identification. The system supports 200+ languages via NLLB-200 translation and provides BibTeX export, similarity metrics, and methodology classification. Evaluation on 15 diverse queries shows 100\% document coverage, an average retrieval score of 7.68, and an LLM-as-Judge quality score of 3.87/5, with a median response time of 10.1 seconds.
\end{abstract}
...
We use the arXiv metadata dataset from Kaggle, filtered to papers in \texttt{cs.CL}, \texttt{cs.AI}, and \texttt{cs.LG} published from 2018 onwards, yielding \textbf{388K papers} with titles, authors, abstracts, and metadata. The 2018 cutoff ensures the corpus reflects the modern transformer era. We additionally extend the corpus with PubMed biomedical abstracts to broaden domain coverage.
...
\begin{enumerate}
  \item \textbf{Language detection \& translation}: The input query's language is detected using \texttt{langdetect}. Non-English queries are translated to English via NLLB-200-distilled-600M~\cite{nllb} for retrieval, and the final response is translated back.
...
Following the project evaluation criteria (Table~1 in the assignment), we measure:

\begin{itemize}
  \item \textbf{RAG System Quality}: LLM-as-Judge scoring, where \texttt{llama3.1:8b} rates each response on a 1--5 scale considering relevance, grounding in retrieved papers, and identification of key differences.
  \item \textbf{Document Coverage}: Percentage of queries for which at least one retrieved paper matches the relevance keywords in its title or abstract.
  \item \textbf{Response Time}: End-to-end latency from query to response.
\end{itemize}
...
\midrule
Document Coverage & 15/15 (100\%) \\
Avg.\ Retrieval Score (cross-encoder) & 7.676 \\
Avg.\ LLM Judge Score & 3.87 / 5 \\
Median Response Time & 10.1s \\
Min / Max Response Time & 7.4s / 27.3s\textsuperscript{*} \\
\bottomrule
\end{tabular}
\vspace{0.3em}

\raggedright\footnotesize\textsuperscript{*}Excluding one outlier (3925.7s) caused by an API timeout on the ``diffusion models for text generation'' query. The mean including this outlier is 299.4s; the median of 10.1s better represents typical performance.
\end{table}
...
\textbf{Evaluation bias.} Our LLM-as-Judge evaluation uses the same model family (\texttt{llama3.1:8b}) for both generation and evaluation, introducing potential self-evaluation bias. A more robust evaluation would use human annotators or a different model family as judge.
